<template>
   <main class="home-page">
      <section class="container mx-auto px-4">
        ...
        <button v-if='$store.state.total_posts > posts.length' @click='$store.dispatch("LoadMorePosts", 1)' class='btn mt-8'>
          Show More {{ $store.state.total_posts - posts.length }}
        </button>
      </section>
   </main>
</template>
      ...
      onMounted(() => {
         ...
         subscription.value = sanity.listen(query).subscribe((update) => {
            switch (update.transition) {
               case 'update':
                  sanity
                     .getDocument(update.result.author._ref)
                     .then((author) => {
